...
- `SHOPPING_MALLS`(원본 배열) 직접 참조는 **다른 형태로 파생할 때만** 허용한다 (예: 같은 파일의 코드 목록 `SHOPPING_MALL_CODES`, `ShoppingMalls` 유니온 타입). 이름 조회·필터 옵션·코드 허용값 목적이면 위 표와 `SHOPPING_MALL_CODES`를 쓴다.
