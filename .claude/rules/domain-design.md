...
**같은 판별로 선택값이 되는 것 — 옵션 수량.** `OptionCombination.quantity`도 어느 몰이 요구하는 값이 아니라 사용자가 재고를 관리하는 방식에 딸린 값이다. 2026-09-04 상품 폼의 필수 필드를 정비하면서 "옵션 필드에 검증이 하나도 없는 것은 누락 아닌가"를 물었고 **선택값이 맞다**고 확인받았다. 옵션 조합의 `quantity`·`optionPrice`·`skuCode` 어느 것도 필수가 아니며, 이 역시 빠뜨린 게 아니라 그렇게 설계된 것이다.
...
설계 근거: `docs/superpowers/specs/2026-09-16-product-customer-code-unique-design.md`

관련: `docs/superpowers/specs/2026-08-22-product-option-id-field-reduction.md`

## 프로젝트 규모 전제 — 포트폴리오 규모, 무료 인프라

이 프로젝트의 목표는 **포트폴리오 규모**이며 실제 운영은 고려하지 않는다(2026-09-13 사용자 확정). 인프라는 전부 무료 요금제다 — Vercel Hobby, Cloudflare R2, Neon.

- **긴 작업·대량 처리는 백그라운드 작업이나 큐로 풀지 않는다.** 대신 **처리 한도를 제품 정책으로 둔다.** 예: 엑셀 대량등록 1회 최대 50건(`PRODUCT_BULK_MAX_ROWS`).
- "실제 운영이라면 이렇게 해야 한다"는 이유만으로 유료 인프라·외부 큐를 설계 전제로 삼지 않는다. 필요해 보이면 포기한 것과 전환 조건을 `docs/solutions/architecture-patterns/`에 기록한다.
- 한도를 둔 처리는 나중에 실행 위치를 바꿀 수 있게 경계를 둔다(예: 행 목록 → 행별 결과를 돌려주는 순수 함수).

**Why:** 엑셀 이미지 R2 가져오기 설계에서 "저장하는 동안 다른 작업을 못 한다"를 제대로 풀려면 백그라운드 작업이 필요했다. 원인은 요금제가 아니라 서버리스 구조였고(유료 요금제에도 계속 도는 작업자는 없다), 규모 가정을 명시하고 그 안에서 설계하는 쪽으로 결론 났다.
