...
**카테고리 선택이 곧 커밋 여부 결정이다.** `architecture-patterns/`만 git에 추적되고 나머지 카테고리(`conventions`·`logic-errors`·`ui-bugs`·`integration-issues`)는 로컬 보관이다. 재사용 가능한 설계 결론이면 `architecture-patterns/`에, 특정 작업의 진행 기록이면 나머지에 쓴다 (`.claude/rules/git.md` 참조).
