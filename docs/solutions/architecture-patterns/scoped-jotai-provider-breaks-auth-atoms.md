...
**상품목록 → 쇼핑몰상품등록 필터 전이는 2026-08-06 기준 아직 고쳐지지 않았다.** 경계 리셋 방식으로 한 번 구현했다가 원복했고(요구사항을 더 확인한 뒤 처리하기로 함), 이 문서는 그 과정에서 확인된 **Provider 방식이 불가능하다는 사실**만 남긴 것이다. 다시 착수할 때 Provider부터 시도하지 말 것.
