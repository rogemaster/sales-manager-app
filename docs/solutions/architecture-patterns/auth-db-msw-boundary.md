...
이 프로젝트는 처음에 개발 환경에서 MSW(Mock Service Worker)로 모든 API를 처리하는 구조였다. 그러나 MSW의 구조적 한계 및 DB와의 일관성 유지 필요로 인해 인증·유저 관련 엔드포인트부터 실제 DB route handler로 전환했다.
...
## 판단 기준 (2026-06-24 ~ 2026-09-13 — 과거 기준)

당시 새 기능을 구현할 때 DB route handler로 전환할지 MSW로 유지할지 판단하던 기준이다. 이후 영속화가 필요한 도메인을 차례로 옮기면서 "route가 기본"으로 뒤집혔다(위 "현재 경계").
...
2026-06-24 시점에는 모든 기능을 DB화하면 작업량·도메인별 스키마 설계 부담이 과하다고 보고, **MSW가 구조적으로 할 수 없는 일만 DB route로 옮기는** 경계를 택했다. MSW는 화면을 먼저 넓게 펼치는 프로토타이핑 단계에 맞았다.

이후 "넓게 펼쳤지만 깊이가 없다"는 판단으로 영역별 DB화에 들어가면서(상품 → 쇼핑몰 → 주문 순) 이 경계는 뒤집혔다. 도메인 테스트는 `@/db`를 모킹해 순수 로직에 붙이므로, 걱정했던 "테스트의 실제 DB 의존"은 생기지 않았다.

**도메인을 하나씩 옮길 때마다 위 "과도기" 절의 비용이 발생한다.** 주문 영역을 옮길 때도 같다. 옮기는 도메인의 데이터를 MSW 잔존 도메인이 읽고 있는지 먼저 grep하고(상품의 경우 홈·연동상품 두 곳이었다), 걸리는 핸들러가 있으면 **그 핸들러도 함께 route로 옮기는 것을 이전 범위에 넣는다.**
