...
| **이미지 표시에 "key는 있는데 객체가 없는" 경우의 대체 표시(fallback)가 필요하다.** 데모 버킷에 **25일 수명주기 규칙**이 걸려 있어(2026-09-01 사용자 설정) 그 이후 업로드 이미지는 객체가 사라진다. 즉 깨진 이미지가 예외 상황이 아니라 **정상적으로 발생하는 상태**다 | **사용자 설정**에서 파생 — 표시 라운드 착수 시 반드시 다룰 것 |
