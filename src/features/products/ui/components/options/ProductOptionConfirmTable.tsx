...
export const ProductOptionConfirmTable = ({ name, optionCombinations, isOptionsConfirmed }: Props) => {
  ...
  const handleOptionTotalQuantity = () => {
    const total = fields.reduce((acc, cur) => acc + (Number(cur.quantity) || 0), 0);
    if (name === 'option') setValue('totalQuantity', total);
  };
      ...
      {isOptionsConfirmed && optionCombinations.length > 0 && (
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>
              생성된 {fields.length}개의 옵션 조합별로 수량과 SKU 코드를 설정하세요.
            </CardDescription>
          </CardHeader>
          <CardContent>
                    ...
                    <div key={field._id} className="p-4 hover:bg-muted/30">
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                        ...
                        <div className="space-y-1">
                          ...
                          <Input placeholder="SKU-001" className="h-8" {...register(`${name}.${index}.skuCode`)} />
                        </div>
                        ...
                        <div className="space-y-1">
                          ...
                          <Input
                            ...
                            placeholder="0"
                            className="h-8"
                            {...register(`${name}.${index}.optionPrice`, { valueAsNumber: true })}
                          />
                        </div>
                      </div>
                    </div>
