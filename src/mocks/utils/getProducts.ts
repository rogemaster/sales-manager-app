import { Product, ProductSearch, ProductSearchType } from '@/features/products/types/product.types';
...
const SEARCH_TARGET: Record<ProductSearchType, (product: Product) => string> = {
  productName: (product) => product.name,
  productCode: (product) => product.productId,
};

const getProductsBysearchValue = (searchType: ProductSearchType, value: string, data: Product[]) => {
  if (!value) {
    return data;
  }
  const target = SEARCH_TARGET[searchType];
  return data.filter((item) => target(item).includes(value));
};

export const getMockProducts = (ownerId: string, searchParams: ProductSearch, page: number, pageSize: number) => {
  const { dateType, startDate, endDate, saleType, categoryId, searchType, searchValue } = searchParams;
  ...
  const filtered = getProductsBysearchValue(searchType, searchValue, resultByCategory);
