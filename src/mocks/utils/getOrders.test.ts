...
describe('getMockOrders', () => {
  describe('필터 없음', () => {
    it('아무 필터도 없으면 전체 주문을 반환한다', () => {
      const result = getMockOrders(defaultFilters, 1, 10);
  ...
  describe('쇼핑몰 필터', () => {
    it("'COUP'만 필터링하면 COUP 주문 2개를 반환한다", () => {
      const result = getMockOrders({ ...defaultFilters, shoppingMall: 'COUP' }, 1, 10);
      expect(result.total).toBe(2);
      result.orders.forEach((o) => expect(o.shoppingMallName).toBe('COUP'));
    });

    it("'NSST'만 필터링하면 NSST 주문 1개를 반환한다", () => {
      const result = getMockOrders({ ...defaultFilters, shoppingMall: 'NSST' }, 1, 10);
      expect(result.total).toBe(1);
      expect(result.orders[0].orderNumber).toBe('ORD-002');
    });

    it("'ALL'이면 전체를 반환한다", () => {
      const result = getMockOrders({ ...defaultFilters, shoppingMall: 'ALL' }, 1, 10);
      expect(result.total).toBe(3);
    });
  });
  ...
  describe('주문 상태 필터', () => {
    it("'NEW_ORDER' 상태만 필터링하면 1개를 반환한다", () => {
      const result = getMockOrders({ ...defaultFilters, orderStatus: 'NEW_ORDER' }, 1, 10);
      expect(result.total).toBe(1);
      expect(result.orders[0].orderNumber).toBe('ORD-001');
    });

    it("'CONFIRMED_ORDER' 상태만 필터링하면 1개를 반환한다", () => {
      const result = getMockOrders({ ...defaultFilters, orderStatus: 'CONFIRMED_ORDER' }, 1, 10);
      expect(result.total).toBe(1);
      expect(result.orders[0].orderNumber).toBe('ORD-002');
    });

    it("'ALL'이면 전체를 반환한다", () => {
      const result = getMockOrders({ ...defaultFilters, orderStatus: 'ALL' }, 1, 10);
      expect(result.total).toBe(3);
    });
  });
  ...
  describe('검색 필터', () => {
    it('searchType orderName으로 검색하면 주문자명이 일치하는 주문을 반환한다', () => {
      const result = getMockOrders({ ...defaultFilters, searchType: 'orderName', searchValue: '홍길동' }, 1, 10);
      expect(result.total).toBe(1);
      expect(result.orders[0].orderNumber).toBe('ORD-001');
    });

    it('searchType orderProductName으로 검색하면 상품명에 키워드가 포함된 주문을 반환한다', () => {
      const result = getMockOrders({ ...defaultFilters, searchType: 'orderProductName', searchValue: '운동화' }, 1, 10);
      expect(result.total).toBe(2); // 나이키 운동화, 뉴발란스 운동화
    });

    it('searchType orderNumber로 검색하면 주문번호가 일치하는 주문을 반환한다', () => {
      const result = getMockOrders({ ...defaultFilters, searchType: 'orderNumber', searchValue: 'ORD-002' }, 1, 10);
      expect(result.total).toBe(1);
      expect(result.orders[0].orderNumber).toBe('ORD-002');
    });

    it('존재하지 않는 searchType이면 전체를 반환한다', () => {
      const result = getMockOrders({ ...defaultFilters, searchType: 'unknown', searchValue: '홍길동' }, 1, 10);
      expect(result.total).toBe(3);
    });

    it('searchValue가 빈 문자열이면 전체를 반환한다', () => {
      const result = getMockOrders({ ...defaultFilters, searchType: 'orderName', searchValue: '' }, 1, 10);
      expect(result.total).toBe(3);
    });
  });
  ...
  describe('날짜 필터', () => {
    it('범위 안의 날짜만 반환한다', () => {
      const result = getMockOrders({ ...defaultFilters, startDate: '2024-01-01', endDate: '2024-01-19' }, 1, 10);
      expect(result.total).toBe(1); // ORD-001 (2024-01-15)만 포함
      expect(result.orders[0].orderNumber).toBe('ORD-001');
    });

    it('dateType이 orderCollectionDate이면 수집일 기준으로 필터링한다', () => {
      const result = getMockOrders(
  ...
  describe('페이지네이션', () => {
    it('page 1, pageSize 2면 첫 두 주문을 반환한다', () => {
      const result = getMockOrders(defaultFilters, 1, 2);
    ...
    it('page 2, pageSize 2면 나머지 주문을 반환한다', () => {
      const result = getMockOrders(defaultFilters, 2, 2);
    ...
    it('결과가 0개면 totalPages가 1이다', () => {
      const result = getMockOrders({ ...defaultFilters, orderStatus: 'REQUEST_CANCEL' }, 1, 10);
      expect(result.total).toBe(0);
      expect(result.totalPages).toBe(1);
    });

    it('응답에 page와 pageSize가 포함된다', () => {
      const result = getMockOrders(defaultFilters, 2, 5);
